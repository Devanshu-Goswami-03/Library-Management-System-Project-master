...
public void clear(){
    ...
    private void btnissueActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnissueActionPerformed
        ...
        if (txtid.getText().equals("")) {
            JOptionPane.showMessageDialog(rootPane, "Please enter Book ID and search it.");
            txtid.requestFocus();
            return;
        }
        ...
        try {
            // 1) Check ki book Available hai ya nahi
            pst = c.prepareStatement("SELECT status FROM book WHERE id = ?");
            ...
            // 2) issuebook table me record insert karo
            pst = c.prepareStatement(
                    "INSERT INTO issuebook (book_id, student_id, date, status, due_date) VALUES (?,?,?,?,?)"
            );
            pst.setString(1, txtid.getText());
            pst.setString(2, txtstudentid.getText());
            pst.setString(3, txtissuedate.getText());  // "dd/MM/yyyy" string
            pst.setString(4, "Issued");
            pst.setString(5, txtduedate.getText());

            pst.executeUpdate();

            // 3) book table me status update karo
            pst = c.prepareStatement("UPDATE book SET status = 'Issued' WHERE id = ?");
            pst.setString(1, txtid.getText());
            pst.executeUpdate();
            ...
            // issue date ko dobara set kar do, clear() ne empty kar diya
            SimpleDateFormat dat = new SimpleDateFormat("dd/MM/yyyy ");
            Date d = new Date();
            txtissuedate.setText(dat.format(d));
            ...
            JOptionPane.showMessageDialog(this, "Error while issuing book: " + ex.getMessage());
        }
